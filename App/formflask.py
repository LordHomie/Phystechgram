...
def messages():
    if 'user_id' in session:
        ...
        user_search = session['user_search']
        room = name
        room_receiver = user_search
        return render_template('messages.html', name=name, room=room, room_receiver=room_receiver)
...
def handle_send_message_event(data):
    app.logger.info("{} has sent message to the room {}: {}".format(data['name'],
                                                                    data['room_receiver'],
                                                                    data['message']))
    socketio.emit('receive_message', data, room=data['room_receiver'])


@socketio.on('send_message1')
def handle_send_message_event1(data):
    app.logger.info("{} has sent message to the room {}: {}".format(data['name'],
                                                                    data['room'],
                                                                    data['message']))
    socketio.emit('receive_message1', data, room=data['room'])
